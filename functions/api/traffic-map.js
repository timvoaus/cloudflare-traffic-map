...
//
// range=24h (default) — returns the atomically published current snapshot,
//                       with legacy tables used before the first publication.
...
function normalizeSnapshot(value) {
  if (!value || !Array.isArray(value.sources) || !Array.isArray(value.destinations) || !Array.isArray(value.routes)) return null;
  return {
    sources: value.sources,
    destinations: value.destinations,
    routes: value.routes.map(r => ({
      sourceCountry: r.sourceCountry ?? r.source_country,
      destinationCountry: r.destinationCountry ?? r.destination_country,
      sourceLat: r.sourceLat ?? r.source_lat,
      sourceLng: r.sourceLng ?? r.source_lng,
      destinationLat: r.destinationLat ?? r.destination_lat,
      destinationLng: r.destinationLng ?? r.destination_lng,
      count: r.count,
    })),
...
export const onRequestGet = async (context) => {
  ...
  const days = Object.hasOwn(RANGE_TO_DAYS, rangeParam) ? RANGE_TO_DAYS[rangeParam] : 1;
  const range = days === 1 ? '24h' : `${days}d`;
  const cache = globalThis.caches?.default;
  const cacheKey = new Request(`${url.origin}${url.pathname}?range=${range}`);
  if (cache) {
    try {
      const cached = await cache.match(cacheKey);
      if (cached) return cached;
    } catch (err) {
      console.warn('traffic-map cache read failed', err);
    }
  }
  const respond = async (body, status = 200, cacheable = status === 200) => {
    const response = new Response(JSON.stringify(body), {
      status, headers: cacheable ? headers : { ...headers, 'cache-control': 'no-store' },
    });
    if (cacheable && cache) {
      try {
        const put = cache.put(cacheKey, response.clone());
        if (typeof context.waitUntil === 'function') context.waitUntil(Promise.resolve(put).catch(err => console.warn('traffic-map cache write failed', err)));
        else await put.catch(err => console.warn('traffic-map cache write failed', err));
      } catch (err) {
        console.warn('traffic-map cache write failed', err);
      }
    }
    return response;
  };
  ...
  try {
    if (days > 1) {
      return await aggregatedResponse(env, days, range, respond);
    }
    const [snapshotRes, historyRes, metaRes, rangeRes] = await Promise.all([
      env.DB.prepare("SELECT value FROM meta WHERE key='current_snapshot'").first(),
    ...
    let snapshot = null;
    if (snapshotRes) {
      snapshot = normalizeSnapshot(JSON.parse(snapshotRes.value));
      if (!snapshot) throw new Error('Invalid current snapshot');
    }
    let legacy = { sources: [], destinations: [], routes: [] };
    if (!snapshot) {
      const [sourcesRes, destsRes, routesRes] = await Promise.all([
        ...
        env.DB.prepare(`SELECT source_country, destination_country, source_lat, source_lng,
                               destination_lat, destination_lng, count
                        FROM routes ORDER BY count DESC`).all(),
      ]);
      legacy = {
        sources: (sourcesRes.results || []).map(r => ({
      country: r.country, lat: r.lat, lng: r.lng, count: r.count,
        })),
        destinations: (destsRes.results || []).map(r => ({
      country: r.country, lat: r.lat, lng: r.lng, count: r.count,
        })),
        routes: (routesRes.results || []).map(r => ({
    ...
    const sources = snapshot?.sources || legacy.sources;
    const destinations = snapshot?.destinations || legacy.destinations;
    const routes = snapshot?.routes || legacy.routes;
    ...
    try { lastRefresh = snapshot?.lastRefresh || (metaRes ? JSON.parse(metaRes.value) : null); } catch {}
...
async function aggregatedResponse(env, days, range, respond) {
  try {
